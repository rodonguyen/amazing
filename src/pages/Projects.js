...
const projectContents = {
	1: {
		short: (
			<p>
				Led a team of 4 to design 4 separate <b>Machine Learning models</b> that can
				identify spam tweets with <b>{'>'}95% accuracy</b> (LSTM, Transformer, SVM and Naive
				Bayes), provided insights into each model’s strengths and shortcomings
			</p>
		),
		long: (
			<>
				<p>
					<ul>
						...
						<li>
							...
							suitability for real-world application: Naive Bayes won due to its low
							training, inference time, and high weighted-F1.
						</li>
						<li>
							Further minimised the computation consumption and sped up training time
							by removing uninformative features while keeping the accuracy
							unaffected.
						</li>
					</ul>
				</p>
			...
			<>
				<p>
					<ul>
						...
						<li>
							Backtest results showed +110% profit annually on average by utilising
							SuperTrend algorithm. This is made possible by programmatically and
							analytically assessing each configuration’s risk-adjusted return and
							profit/loss distribution.
						</li>
						<li>
							Applied OOP to facilitate maintainability and extensibility as a large
							number of features were integrated into the bot.
						</li>
						<li>
							Planning to incorporate Deep Learning for smarter trading strategy and
							build a User Interface for easier operation.
						</li>
					</ul>
				</p>
			</>
	...
	3: {
		short: (
			<p>
				...
				<b>persistance services</b> that analyses per-keyword sentiment on Twitter using
				Twitter REST API
			</p>
		),
		long: (
			<>
				<p>
					<ul>
						...
						<li>
							Implemented scalable server using AWS EC2, Elastic Load Balancing and
							Auto Scaling stack to handle inconsistent usage, data were queried via
							Twitter API.
						</li>
						<li>
	...
	4: {
		short: (
			<p>
				...
				your life differently. <b>#TypeScript</b>.
			</p>
		),
		long: (
			<p>
	...
	5: {
		short: (
			<p>
				Programmed from scratch with React. Cutomised CSS to obtain responsiveness on any
				device (no Bootstrap). Of course, integrated Dark Mode to accommodate users with
		...
		long: (
			<p>
				<ul>
					<li>Programmed and deployed my personal website from scratch with React.</li>
					<li>
						Cutomised CSS to obtain responsiveness on any device (no Bootstrap). Of
						course, integrated Dark Mode to accommodate users with photophobia or simply
						prefer dark theme.
					</li>
			...
			<p>
				<ul>
					...
					<li>
						...
						suitable database schemas, implemented multi-step forms and a multitude of
						API endpoints, and utilised React Hooks (e.g., useMemo), all of which
...
const projectContentHandler = (projects, setProjects, target) => {
	...
	} else if (projects.expandedTarget !== '-1' && projects.expandedTarget !== target) {
		projects[projects.expandedTarget] = projectContents[projects.expandedTarget].short; // Shorten the previous project
...
export default function Projects() {
	...
	function createProjectEntry(name, projectIndex, githubURL, projectImage, extraButton = null) {
		return (
	...
	// /** Create a row of project including its thumbnail image, urls, description and buttons */
	// function createProjectEntry02(name, projectIndex, githubURL, projectImage, extraButton = null) {
	// 	return (
	// 		<div className='project'>
	// 			{createHyperlink(
	// 				githubURL,
